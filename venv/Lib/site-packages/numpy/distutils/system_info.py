...
if sys.platform == "win32":
    ...
    try:
        ...
        p = sp.Popen(["gcc", "-print-multiarch"], stdout=sp.PIPE, stderr=tmp)
...
class atlas_3_10_blas_info(atlas_3_10_info):
    ...
    def calc_info(self):
        lib_dirs = self.get_lib_dirs()
        info = {}
        atlas_libs = self.get_libs("atlas_libs", self._lib_names)
        atlas = self.check_libs2(lib_dirs, atlas_libs, [])
        if atlas is None:
            return
        include_dirs = self.get_include_dirs()
...
""",
        ...
        if atlas_version:
            dict_append(
                info, define_macros=[("ATLAS_INFO", '"\\"%s\\""' % atlas_version)]
            )
        else:
    ...
    def calc_info(self):
        ...
        for v in ["__version__", "version"]:
            ...
            if vrs is None:
                continue
            macros = [
                (self.modulename.upper() + "_VERSION", '"\\"%s\\""' % (vrs)),
