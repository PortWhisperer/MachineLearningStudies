...
def test_nested_exception_dispatch(backend):
    ...
    if PY27 and backend == "multiprocessing":
        raise SkipTest(
            "Nested parallel calls can deadlock with the python 2.7"
...
@pytest.mark.parametrize(
    "backend", ["threading", "loky", "multiprocessing", "back_compat_backend"]
)
...
def test_no_blas_crash_or_freeze_with_subprocesses(backend):
    if backend == "multiprocessing":
        if sys.version_info < (3, 4):
            raise SkipTest(
                "multiprocessing can cause BLAS freeze on old "
...
@parametrize(
    "backend",
    PROCESS_BACKENDS
    + ([] if sys.version_info[:2] < (3, 4) or mp is None else ["spawn"]),
)
