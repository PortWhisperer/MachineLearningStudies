...
def test_inheritance():
    ...
    assert isinstance(my_exceptions.JoblibNameError(), my_exceptions.JoblibException)
    assert my_exceptions.JoblibNameError is my_exceptions._mk_exception(NameError)[0]


def test_inheritance_special_cases():
    # _mk_exception should transform Exception to JoblibException
    assert my_exceptions._mk_exception(Exception)[0] is my_exceptions.JoblibException
    ...
    for exception_type in [
        ...
        assert my_exceptions._mk_exception(exception_type)[0] is exception_type
    ...
    for exception in non_inheritable_classes:
        assert (
            my_exceptions._mk_exception(exception)[0] is my_exceptions.JoblibException
        )
