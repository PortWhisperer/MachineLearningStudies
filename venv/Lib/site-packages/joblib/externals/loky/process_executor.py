...
def _python_exit():
    ...
    mp.util.debug(
        "Interpreter shutting down. Waking up queue_manager_threads " "{}".format(items)
    )
...
class _SafeQueue(Queue):
    ...
    def _on_queue_feeder_error(self, e, obj):
        if isinstance(obj, _CallItem):
            ...
            raised_error = set_cause(
                raised_error, _RemoteTraceback('\n"""\n{}"""'.format("".join(tb)))
            )
...
def _add_call_item_to_queue(
    pending_work_items, running_work_items, work_ids, call_queue
):
...
def _queue_management_worker(
    ...
    def is_shutting_down():
        ...
        return _global_shutdown or (
            (executor is None or executor_flags.shutdown) and not executor_flags.broken
        )
    ...
    def shutdown_all_workers():
        ...
        while n_sentinels_sent < n_children_to_stop and n_children_alive > 0:
            for i in range(n_children_to_stop - n_sentinels_sent):
                try:
                    ...
                    break
            with processes_management_lock:
                n_children_alive = sum(p.is_alive() for p in list(processes.values()))
        ...
        while processes:
            ...
            p.join()
        mp.util.debug(
            "queue management thread clean shutdown of worker "
    ...
    while True:
        _add_call_item_to_queue(
            pending_work_items, running_work_items, work_ids_queue, call_queue
        )
        ...
        if result_reader in ready:
            try:
                ...
                if isinstance(result_item, _RemoteTraceback):
                    broken = (
                        "A task has failed to un-serialize. Please "
                ...
                if tb is None:
                    _, _, tb = sys.exc_info()
                broken = (
                    "A result has failed to un-serialize. Please "
        ...
        if isinstance(result_item, int):
            ...
            if n_pending - n_running > 0 or n_running > len(processes):
                ...
                if executor is not None and len(processes) < executor._max_workers:
                    warnings.warn(
