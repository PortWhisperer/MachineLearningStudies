...
def eval_Knn_Performance(var_list):
    if len(var_list) == 6:
        ...
        xi, xi_test = (
            x_i,
            x_i_test,
        )  # else pull in x values from global scope and avoid shadowing
    ...
    for K in range(1, 15):
        ...
        error = sqrt(
            mean_squared_error(y_i_test, pred)
        )  # calculate rmse against test data using y hat, not f(x)
        error_train = sqrt(
            mean_squared_error(y_i, pred_train)
    ...
    lin_reg_model_pred_train = (
        lin_reg_model_train[0] * np.linspace(-1, 1, 100) + lin_reg_model_train[1]
    )
    lin_reg_model_pred_test = (
        lin_reg_model_test[0] * np.linspace(-1, 1, 10000) + lin_reg_model_test[1]
    )

    error_lin_reg_test = sqrt(
        mean_squared_error(y_i_test, lin_reg_model_pred_test)
    )  # out of sample RMSE = 1.0233
    error_lin_reg_train = sqrt(
        mean_squared_error(y_i, lin_reg_model_pred_train)
    )  # in sample RMSE = .93796

    # plot results
    # RMSE vs log(k)
    k = np.linspace(2.0, 15.0, 14)  # create for matplotlib plotting purposes
    plt.figure(1)  # generate sequential figure numbers to get more plots
    plt.subplot(2, 1, 1)
    plt.suptitle(op_name + "RMSE analysis")
    plt.plot(np.log(1.0 / k), np.array(rmse_val), c="r", label="RMSE")
    plt.plot(
        np.log(1.0 / k), np.array(rmse_val_train), c="black", label="In sample RMSE"
    )
    plt.axhline(y=error_lin_reg_test, c="g", dashes=(5, 1), label="Lin Reg RMSE")
    plt.xlabel("Log(1/k)")
    plt.ylabel("RMSE")
    plt.title("Performance versus log(1/k)")
    plt.legend(loc="upper left")

    # RMSE vs K
    plt.subplot(2, 1, 2)
    plt.plot(k, np.array(rmse_val), c="r", label="RMSE")
    plt.plot(k, np.array(rmse_val_train), c="black", label="In sample RMSE")
    plt.axhline(y=error_lin_reg_test, c="g", dashes=(5, 1), label="Lin Reg RMSE")
    plt.xlabel("K")
    plt.ylabel("RMSE")
    ...
    plt.figure(2)  # change figure number for upcoming plot generation
    for i, n_neighbors in enumerate([2, 5, 10, 12]):
        ...
        plt.subplot(4, 1, i + 1)
        plt.scatter(
            xi_test.reshape(-1, 1), y_i_test.reshape(-1, 1), c="grey", label="test data"
        )
        ...
        plt.legend()
        plt.title(
            "KNeighborsRegressor (k = %i, weights = '%s')" % (n_neighbors, "uniform")
        )
        # plot f(x) for comparison
        plt.plot(
            true_fx_x_vals, true_fx_y_vals, "r--", label="f"
...
# given f(x) = 1.8*x_i + 2
x_i, x_i_test = (
    np.sort(np.random.uniform(-1, 1, 100), axis=0),
...
e_i, e_i_test = (np.random.normal(0, 1, 100), np.random.normal(0, 1, 10000))
...
true_func_y_vals_sin_poly = (
    1.8 * true_func_x_vals + 2
)  # placeholder for eventual polynomial func
...
arr.append(
    (
        1.8 * x_i + 2 + e_i,
        1.8 * x_i_test + 2 + e_i_test,
        true_func_y_vals_lin,
        "Linear Reg",
    )
)
arr.append(
    (
        np.tan(1.1 * x_i) + 2 + e_i,
        np.tan(1.1 * x_i_test) + 2 + e_i_test,
        true_func_y_vals_tan,
        "Tan Func",
    )
)
arr.append(
    (
        np.sin(2 * x_i) + 2 + e_i,
        np.sin(2 * x_i_test) + 2 + e_i_test,
        true_func_y_vals_sin,
        "Sin Func",
    )
)
